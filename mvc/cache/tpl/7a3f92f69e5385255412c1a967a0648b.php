    ...
    <div class="topnews">
        <h2>
           ...
           <?php if(!empty($bloginfo)): ?>
            <?php foreach($bloginfo as $key => $val) :?>
                <div class="blogs">
        <figure>
            <a href="https://www.yezismile.com/detail/457.html" title="<?=$val['title'];?>" target="_blank">
            <img src="public/images/201710091321426416_s.jpg" original="https://static.yezismile.com/data/photo/day_20171107/201711072145543410_s.jpeg" style="width:167.5px;height:137px;" alt="<?=$val['title'];?>" >
            </a>
        </figure>
         <ul>
          <h3><a href="index.php?m=article&a=article&id=<?=$val['id'];?>" title="<?=$val['title'];?>" target="_blank"><?=$val['title'];?></a></h3>
          <p>
          <?=$val['content'];?>  <!-- Baidu Button BEGIN -->
...
document.getElementById("bdshell_js").src = "http://bdimg.share.baidu.com/static/js/shell_v2.js?cdnversion=" + Math.ceil(new Date()/3600000)
    ...
    <!-- Baidu Button END -->
          ...
          <p class="autor">
            ...
            <span class="dtime f_l"><?php echo date('Y-m-d', $val['addtime']);?></span>
			...
			<span class="label_bottom f_r" style="padding-left: 0;">
			    <a href="index.php?m=thumb&a=thumb&id=<?=$val['id'];?>"  class="yz_zan" style=""><?php foreach($thumbinfo as $value) :?><?php if($val['id'] == $value['tid']): ?><?=$value['t'];?><?php endif;?><?php endforeach;?></a>
			</span>
            <span class="viewnum f_r">浏览（<?=$val['hits'];?>）</span>
             
             <span ><a href="index.php?m=thumb&a=favirote&id=<?=$val['id'];?>">收藏 <?php foreach($faviroteinfo as $value) :?><?php if($val['id'] == $value['tid']): ?><?=$value['t'];?><?php endif;?><?php endforeach;?></a></span>
            <span class="pingl f_r">
            	
              <a  href="index.php?m=article&a=article&id=<?=$val['id'];?>" >
				  <span id = "sourceId::457" class = "cy_cmt_count" ><?php foreach($replyinfo as $value) :?><?php if($val['id'] == $value['replyid']): ?><?=$value['replycount'];?><?php endif;?><?php endforeach;?></span>
				</a>
			</span>
          </p>
        </ul>
